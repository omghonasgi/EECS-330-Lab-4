...
{  
  private:
    ...
    {
        // When expanding, ensure we have enough capacity
    ...
    void reserve(size_t newCapacity)
    {
        ...
        // Copy elements in their correct order
  ...
  public:
    ...
    {
        if (this->empty()) {
            // If queue is empty, reset indices
            this->resize(1);
            dataStart = 0;
            dataEnd = 0;
            this->data[0] = x;
        } else {
            // Add new element and update dataEnd
    ...
    {
        if (!this->empty()) {
            // Move dataStart and decrease size
